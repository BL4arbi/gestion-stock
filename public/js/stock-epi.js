...
async function loadProductsEPI() {
  try {
    const response = await fetch(`${API}/products?category=epi`, { credentials: "include" });
  ...
  container.innerHTML = products.map(p => `
    <div class="product-card ${p.quantite <= p.seuil_alert ? 'alert' : ''}">
      <div class="product-header">
        <h3>${p.nom}</h3>
        ${p.quantite <= p.seuil_alert ? '<span class="badge-alert">⚠️ Stock faible</span>' : ''}
      </div>
      ...
      <div class="product-actions">
        <button class="btn-action" onclick="openEditModalEPI(${p.id})">✏️ Modifier</button>
        <button class="btn-delete" onclick="deleteProductEPI(${p.id})">🗑️ Supprimer</button>
      </div>
    </div>
  `).join("");
}
  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width:500px;">
      ...
      <form onsubmit="saveEditEPI(event, ${id})">
        <div class="form-group"><label>Nom</label><input type="text" id="edit-nom-epi" value="${product.nom}" required /></div>
        <div class="form-group"><label>Quantité</label><input type="number" id="edit-quantite-epi" value="${product.quantite}" required /></div>
        <div class="form-group"><label>Localisation</label><input type="text" id="edit-localisation-epi" value="${product.localisation || ''}" /></div>
        <div class="form-group"><label>Prix</label><input type="number" id="edit-prix-epi" value="${product.prix || 0}" step="0.01" /></div>
        <div class="form-group"><label>Seuil</label><input type="number" id="edit-seuil-epi" value="${product.seuil_alert || 10}" /></div>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (searchInput) searchInput.addEventListener("input", (e) => {
    ...
    const filtered = allProductsEPI.filter(p => p.nom.toLowerCase().includes(text) || (p.localisation && p.localisation.toLowerCase().includes(text)));
